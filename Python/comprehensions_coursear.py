...
def map_id_to_initial(employee_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   new_emp_list = [x['name'] for x in employee_list]
   best_emp_list = [x[0] for x in new_emp_list]
   id_emp_list = [x['id'] for x in employee_list]
   numdict={key:value for (key,value) in zip(best_emp_list,id_emp_list)}
   return numdict
